    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            if ($this->has('start_time') && $this->has('end_time') && $this->has('day') && $this->has('teaching_assignment_id')) {
                ...
                $end = $this->end_time;
                $room = $this->room;
                $timetableId = $this->route('timetable') instanceof Timetable ? $this->route('timetable')->id : $this->route('timetable');

                // Find overlapping timetables for the same day (exclude current and INACTIVE)
